...
async function main() {
  ...
  let nftMarket;
  //proxyAddress is first parameter
  nftMarket = await upgrades.upgradeProxy(
    "0x2016D0a7ac6A1566E4231A8BF4cDFB81BeDF2f56",
    NFTMarket_V2
  );
  console.log("nftMarket upgraded Successfully at: ", nftMarket.address);
  //

  const NFT_V2 = await hre.ethers.getContractFactory("NFT_V2");
  let nft;
  //proxyAddress is first parameter
  nft = await upgrades.upgradeProxy(
    "0x727E2A605c3476684D2b4ee1Ee89887067284a30",
    NFT_V2
  );
  console.log("nft upgraded Successfully at: ", nft.address);
}
